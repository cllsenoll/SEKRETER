    ...
    try:
        ...
        if df is not None:
            ...
            for col in df.columns:
                ...
                if "personel" in col_clean and ("adı" in col_clean or "adi" in col_clean):
                    rename_map[col] = "Personel"
                elif col_clean == "personel":
                    rename_map[col] = "Personel"
                elif "nakit" in col_clean and "ft" in col_clean:
                    rename_map[col] = "Nakit Ft. Tutarı Top"
                elif "nakit" in col_clean and "ödeme" in col_clean:
...
if df is None:
    ...
    def clean_numeric_series(series):
        series_str = series.astype(str).str.replace('\xa0', ' ').str.strip()
        cleaned = series_str.str.replace('TL', '', case=False, regex=False)
    ...
    # Aynı personele ait satırları gruplayarak tutarları topla
